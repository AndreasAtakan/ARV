...
create or replace procedure ar5_overlapp(type text, arealtyper text)
language plpgsql
as $$
begin
	...
	execute format('
		...
		from
			...
			"ar5_%s" as overlapp
		where
			plan.kommunenummer = overlapp.kommunenummer and
			overlapp.arealtype::integer in ( %s ) and
			ST_Intersects( plan.geom, overlapp.geom )
		;
	',
	type,
	( select fylkesnummer from "__planer" limit 1 ),
	arealtyper);
