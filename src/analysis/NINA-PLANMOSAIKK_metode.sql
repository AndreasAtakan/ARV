...
/* Fjern alle overlappende områder fra reguleringsplan
   mellom reguleringsplan og kommuneplan hvor kommuneplan har
   arealstatus lik 2 og reguleringsplan er eldre enn 2008 */
update "__reguleringsplan" as reg
set geom = ST_Multi( ST_CollectionExtract( ST_Difference( reg.geom, plan.geom ), 3 ) )
from
	( select ST_Union(geom) as geom from "__planer" where arealstatus = 2 ) as plan
where
	ST_Intersects( reg.geom, plan.geom ) and
	EXTRACT( year from reg.ikrafttredelsesdato ) < 2008
;
...
where ST_Intersects( plan.geom, reg.geom )
;
