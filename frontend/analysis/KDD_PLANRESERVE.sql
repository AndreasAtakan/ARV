...
/*
	...
	Input tabeller:
	- Plandata finnes i tabell ved navn "__planer"
...
/* Fjern områder som overlapper med AR5 Bebygd/Samferdsel */
update "__planer" as plan
set geom = ST_Multi( ST_CollectionExtract( ST_Difference( plan.geom, omr.geom ), 3 ) )
from
	(
		select ST_Union(geom) as geom
		from "ar5_%f" /* %f get replaced in PHP code */
		where
			kommunenummer = '%k' and
			arealtype IN ('11', '12')
	) as omr
...
/* Fjern områder som overlapper med FKB-Bygning, -Veg og -Tiltak, som er Bolig, Fritidsbebyggelse eller Næringsvirksomhet */
/*update "__planer" as plan
set geom = ST_Multi( ST_CollectionExtract( ST_Difference( plan.geom, omr.geom ), 3 ) )
from
...
/* Fjern områder mindre enn X basert på formålsgruppe */
...
delete from "__planer" as plan
where
	arealformaalsgruppe in ('01 Bolig eller sentrumsformål', '02 Fritidsbebyggelse', '11 Samferdselsanlegg') and
	ST_Area(geom) < 250
;

delete from "__planer" as plan
where
	arealformaalsgruppe = '06 Næringsvirksomhet' and
	ST_Area(geom) < 1000
;

delete from "__planer" as plan
where
	arealformaalsgruppe not in ('01 Bolig eller sentrumsformål', '02 Fritidsbebyggelse', '06 Næringsvirksomhet', '11 Samferdselsanlegg') and
	ST_Area(geom) < 150
;
...
where
	planlegging_status = 'Tidligere plan' and
	(
		ST_IsEmpty(geom) or
		geom is null or
